...
void ComplexFitSamples4(){

    //OPEN ROOT FILE

    gSystem->RedirectOutput("../logs/ComplexFitSamples4.log", "w");
    ...
    TH1F *h1 = new TH1F("h1", "Generated Distribution; Var; Counts", 700, 260.0, 325.0);
    t->Draw("var4 >>h1", "", "goff");
    h1->SetDirectory(0);
    f->Close();
    delete f;
    ...
    RooRealVar var("var4", "Variable", 260.0, 325.0);
    ...
    RooRealVar mean("mean", "Mean of Signal Component", 300.0, 290.0, 315.0);
    RooRealVar sigma1("sigma1", "Width of Signal Component 1", 3.0, 0.1, 20.0);
    RooRealVar alpha1("alpha1", "Alpha of Signal Component 1", 1.0, -10.0, 10.0);
    RooRealVar n1("n1", "n of Signal Component 1", 5.0, 0.1, 20.0);

    RooRealVar sigma2("sigma2", "Width of Signal Component 2", 7.0, 0.1, 20.0);

    RooRealVar frac("frac", "Fraction of Signal Component 1", 0.5, 0.0, 1.0);

    // BACKGROUND PARAMETERS

    RooRealVar tau("tau", "Slope of Exponential Background", -0.05, -1.0, 0.0);
    RooRealVar fracbkg("fracbkg", "Fraction of Exponential Background", 0.5, 0.0, 1.0);

    // SIGNAL AND BACKGROUND PDFS

    RooGaussian signal1("sig1", "Signal Component 1", var, mean, sigma1);
    RooGaussian signal2("sig2", "Signal Component 2", var, mean, sigma2);
    RooAddPdf signal("signal", "Total Signal", RooArgList(signal1, signal2), frac);
    //RooExponential background1("background1", "Exponential Background 1", var, tau);
    //RooPolynomial background2("background2", "Polynomial Background 2", var, RooArgList());
    //RooAddPdf background("background", "Total Background", RooArgList(background1, background2), fracbkg);
    RooExponential background("background", "Exponential Background", var, tau);
    ...
    cmain1->SaveAs("../Png_files/ComplexFitSamples4_results.png");
    cmain2->SaveAs("../Png_files/ComplexFitSamples4_pull.png");

    ofstream out("../Txt_files/ComplexFitSamples4_results.txt");
    ...
    out << "Alpha1: "<< alpha1.getVal()<< " ± "<< alpha1.getError()<< endl;
    out << "n1: "<< n1.getVal()<< " ± "<< n1.getError()<< endl;
