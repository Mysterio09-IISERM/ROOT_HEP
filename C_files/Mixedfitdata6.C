...
void Mixedfitdata6(){
    ...
    TH1F *h1 = new TH1F("h1", "Generated Distribution; Var; Counts", 100, 10.0, 110.0);
    t->Draw("smeared_bw >>h1", "", "goff");
    h1->SetDirectory(0);
    f->Close();
    delete f;
    ...
    RooRealVar var("smeared_bw", "Variable", 10.0, 110.0);
    ...
    RooRealVar mean("mean", "Mean", 90.0, 85.0, 95.0);
    RooRealVar width("width", "Width", 2.5, 0.1, 10.0);
    RooRealVar sigma("sigma", "Smearing Sigma", 2.0, 0.1, 10.0);
    // BACKGROUND PARAMETERS

    RooRealVar c0("c0", "Cheby Coeff 0", 0.0, -1.0, 1.0);
    RooRealVar c1("c1", "Cheby Coeff 1", 0.0, -1.0, 1.0);
    RooRealVar c2("c2", "Cheby Coeff 2", 0.0, -1.0, 1.0);

    // SIGNAL AND BACKGROUND PDFS

    RooVoigtian signal("signal", "Voigtian PDF", var, mean, width, sigma);
    RooChebychev background("background", "Background PDF", var, RooArgList(c0, c1, c2));
    ...
    cmain->SaveAs("../Png_files/mixed_fit6.png");

    ofstream out("../Txt_files/mixed_fit6_results.txt");

    out << "Fit Results:" << endl;
    out << "Signal Yield: " << nsig.getVal() << " ± " << nsig.getError() << endl;
    out << "BW Mean: " << mean.getVal() << " ± " << mean.getError() << endl;
    out << "BW Width: " << width.getVal() << " ± " << width.getError() << endl;
    out << "Smearing Sigma: " << sigma.getVal() << " ± " << sigma.getError() << endl;
