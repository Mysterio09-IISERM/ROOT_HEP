...
void Mixedfitdata3(){
    ...
    TH1F *h1 = new TH1F("h1", "Generated Distribution; Var; Counts", 100, 20.0, 80.0);
    t->Draw("doublegauss_poly >>h1", "", "goff");
    h1->SetDirectory(0);
    f->Close();
    delete f;
    ...
    RooRealVar var("doublegauss_poly", "Variable", 20.0, 80.0);
    ...
    RooRealVar mean1("mean1", "Mean 1", 40.0, 35.0, 55.0);
    RooRealVar sigma1("sigma1", "Width 1", 2.5, 0.1, 10.0);
    RooRealVar mean2("mean2", "Mean 2", 45.0, 35.0, 60.0);
    RooRealVar sigma2("sigma2", "Width 2", 5.0, 0.1, 10.0);

    // BACKGROUND PARAMETERS

    RooRealVar c0("c0", "Cheby Coeff 0", 0.0, -1.0, 1.0);
    ...
    RooRealVar nsig("nsig", "Signal Yield", 58000, 10000, 150000);
    RooRealVar nbkg("nbkg", "Background Yield", 55000, 10000, 150000);
    ...
    TF1 *fsig1 = new TF1("fsig1", "gaus", 20.0, 80.0);
    double scale1 = nsig.getVal() * hsig->GetBinWidth(1) * frac.getVal() / (sigma1.getVal() * sqrt(2 * TMath::Pi()));
    double scale2 = nsig.getVal() * hsig->GetBinWidth(1) * (1 - frac.getVal())/(sigma2.getVal() * sqrt(2 * TMath::Pi()));
    fsig1->SetParameters(scale1, mean1.getVal(), sigma1.getVal());
    TF1 *fsig2 = new TF1("fsig2", "gaus", 20.0, 80.0);
    fsig2->SetParameters(scale2, mean2.getVal(), sigma2.getVal());
    TF1 *ftotal = new TF1("ftotal", "gaus(0) + gaus(3)", 20.0, 80.0);
    ftotal->SetParameters(scale1, mean1.getVal(), sigma1.getVal(), scale2, mean2.getVal(), sigma2.getVal());
    ...
    cmain->SaveAs("../Png_files/mixed_fit3.png");

    ofstream out("../Txt_files/mixed_fit3_results.txt");
    ...
    out << "Width2: " << sigma2.getVal() << " ± " << sigma2.getError() << endl;
