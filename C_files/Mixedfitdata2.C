...
void Mixedfitdata2(){
    ...
    TH1F *h1 = new TH1F("h1", "Generated Distribution; Var; Counts", 100, 50.0, 130.0);
    t->Draw("bw_exp >>h1", "", "goff");
    h1->SetDirectory(0);
    f->Close();
    delete f;
    ...
    RooRealVar var("bw_exp", "Variable", 50.0, 130.0);
    ...
    RooRealVar mean("mean", "Mean", 90.0, 80.0, 100.0);
    ...
    RooBreitWigner signal("signal", "Signal PDF", var, mean, width);
    ...
    TF1 *fsig = new TF1("fsig", "breitwigner", 50.0, 130.0);
    double scale = nsig.getVal() * hsig->GetBinWidth(1);
    fsig->SetParameters(scale, mean.getVal(), width.getVal());
    ...
    c1->SaveAs("../Png_files/mixed_fit2.png");

    ofstream out("../Txt_files/mixed_fit2_results.txt");

    out << "Fit Results:" << endl;
    out << "Signal Yield: " << nsig.getVal() << " ± " << nsig.getError() << endl;
    out << "Mean: " << mean.getVal() << " ± " << mean.getError() << endl;
    out << "Width: " << width.getVal() << " ± " << width.getError() << endl;
