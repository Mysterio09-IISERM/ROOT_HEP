...
void pure_signal(){
    ...
    gSystem->RedirectOutput("../logs/pure_signal.log", "w");

    TFile *f = new TFile("../Root_files/pure_signal.root", "READ");
    TTree *t = (TTree*)f->Get("tree");

    //CREATE HISTOGRAMS

    TH1F *h1 = new TH1F("h1", "Generated Distribution; Var; Counts", 500, 35.0, 55.0);
    t->Draw("x >>h1", "", "goff");
    h1->SetDirectory(0);
    f->Close();
    delete f;
    ...
    RooRealVar var("var", "Variable", 35.0, 55.0);
    ...
    RooRealVar mean("mean", "Mean", 45.0, 42.0, 48.0);
    RooRealVar sigma("sigma", "Sigma", 0.7, 0.1, 5.0);
    ...
    RooAddPdf model("model", "Signal Model", RooArgList(signal), RooArgList(nsig));
...
*/
    ...
    cmain1->SaveAs("../Png_files/pure_signal_results.png");
    cmain2->SaveAs("../Png_files/pure_signal_pull.png");

    ofstream out("../Txt_files/pure_signal_results.txt");
