...
void pure_background(){

    //OPEN ROOT FILE

    gSystem->RedirectOutput("../logs/pure_background.log", "w");

    TFile *f = new TFile("../Root_files/pure_background.root", "READ");
    TTree *t = (TTree*)f->Get("tree");

    //CREATE HISTOGRAMS

    TH1F *h1 = new TH1F("h1", "Generated Distribution; Var; Counts", 500, 0.0, 95.0);
    t->Draw("x >>h1", "", "goff");
    h1->SetDirectory(0);
    f->Close();
    delete f;
    ...
    RooRealVar var("var", "Variable", 0.0, 95.0);
    ...
    RooRealVar tau("tau", "Expo Coeff", -0.05, -1.0, 0.0);
    ...
    //RooAddPdf model("model", "Signal + Background Model", RooArgList(signal, background), RooArgList(nsig, nbkg)); Only signal component for pure signal case

    RooAddPdf model("model", "Background Model", RooArgList(background), RooArgList(nbkg));
...
*/
    ...
    cmain2->cd(2);
    TH1F *hpull = new TH1F("hpull", "Pull Values; Pull; Bins",40, -5, 5);
    for (int i = 0; i < pullHist->GetN(); i++) {
        double x, y;
    ...
    cmain1->SaveAs("../Png_files/pure_background_results.png");
    cmain2->SaveAs("../Png_files/pure_background_pull.png");

    ofstream out("../Txt_files/pure_background_results.txt");
