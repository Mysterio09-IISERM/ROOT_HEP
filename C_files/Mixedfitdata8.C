...
void Mixedfitdata8(){
    ...
    TH1F *h1 = new TH1F("h1", "Generated Distribution; Var; Counts", 100, 20.0, 100.0);
    t->Draw("ugly_bkg >>h1", "", "goff");
    h1->SetDirectory(0);
    f->Close();
    delete f;
    ...
    RooRealVar var("ugly_bkg", "Variable", 20.0, 100.0);
    ...
    RooRealVar mean("mean", "Mean", 54.0, 40.0, 70.0);
    RooRealVar sigma("sigma", "Sigma", 15.0, 5.0, 30.0);

    // BACKGROUND PARAMETERS

    RooRealVar tau("tau", "Expo Coeff", -0.5, -1.0, 0.0);
    ...
    RooPolynomial uniform("uniform", "Unifirom bkg", var);
    RooExponential exponential("exponential", "Exponential bkg", var, tau);
    RooAddPdf background("background", "Background PDF", RooArgList(exponential, uniform), frac);
    ...
    model.plotOn(frame2, Components(exponential), LineColor(kBlue), Name("expbkg"), LineStyle(kDashed));
    model.plotOn(frame2, Components(uniform), LineColor(kMagenta), Name("unibkg"), LineStyle(kDotted));
    ...
    leg2->AddEntry(frame2->findObject("expbkg"), "Exponential Bkg", "l");
    leg2->AddEntry(frame2->findObject("unibkg"), "Uniform Bkg", "l");
    leg2->Draw();
    ...
    cmain->SaveAs("../Png_files/mixed_fit8.png");

    ofstream out("../Txt_files/mixed_fit8_results.txt");
    ...
    out << "Background Fraction: " << frac.getVal() << " ± " << frac.getError() << endl;
