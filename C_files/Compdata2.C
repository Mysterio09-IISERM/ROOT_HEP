...
void Compdata2(){
    ...
    gSystem->RedirectOutput("../logs/Compdata2.log", "w");
    ...
    TH1F *h1 = new TH1F("h1", "Generated Distribution; Var; Counts", 800, 65.0, 105.0);
    t->Draw("var2 >>h1", "", "goff");
    h1->SetDirectory(0);
    f->Close();
    delete f;
    ...
    RooRealVar var("var2", "Variable", 65.0, 105.0);
    ...
    RooRealVar mean1("mean", "Mean", 80.0, 75.0, 85.0);
    RooRealVar sigma1("sigma", "Sigma", 2.0, 0.1, 15.0);

    RooRealVar mean2("mean2", "Mean2", 90.0, 85.0, 95.0);
    RooRealVar width2("width2", "Width2", 0.5, 0.1, 15.0);
    ...
    //RooGaussian signal1("signal1", "Signal Gaussian", var, mean1, sigma1);
    RooBreitWigner signal1("signal1", "Signal Breit-Wigner", var, mean1, sigma1);
    RooBreitWigner signal2("signal2", "Signal Breit-Wigner", var, mean2, width2);
    RooAddPdf signal("signal", "Combined Signal", RooArgList(signal1, signal2), frac);
    RooPolynomial background("background", "Background", var, RooArgList());
    ...
    signal1.plotOn(frame3, Name("sig1"), LineColor(kMagenta), LineStyle(kDotted), Normalization(nsig.getVal()*frac.getVal(),RooAbsReal::NumEvent));
    signal2.plotOn(frame3, Name("sig2"), LineColor(kCyan), LineStyle(kDotted), Normalization(nsig.getVal()*(1-frac.getVal()),RooAbsReal::NumEvent));
    ...
    cmain1->SaveAs("../Png_files/Compdata2_results.png");
    cmain2->SaveAs("../Png_files/Compdata2_pull.png");

    ofstream out("../Txt_files/Compdata2_results.txt");
